...
$driverRoot = 'C:\Windows\System32\drivers'
$destinationRoot = Join-Path $outputRoot 'drivers'
...
foreach ($driver in $drivers) {
    ...
    $manifest += [pscustomobject]@{
        ...
        output = Join-Path 'drivers' $relative
...
$manifest | ConvertTo-Json -Depth 4 |
    Out-File (Join-Path $outputRoot 'serviced-driver-set-manifest.json') -Encoding utf8
New-Item (Join-Path $outputRoot 'SERVICED-DRIVERS-DONE.TAG') -ItemType File -Force | Out-Null
shutdown.exe /s /t 5
