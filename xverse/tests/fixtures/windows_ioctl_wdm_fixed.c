/*
 * Benign, never-loaded WDM-shaped fully bounded control fixture.
 *
 * This owned fixture proves three independent conditions before its inert copy:
 * the four-byte length field exists, the requested source span fits inside the
 * remaining METHOD_BUFFERED input, and the destination span fits KernelSink.
 * It is compiled only for static PyGhidra evidence tests. It exposes no device,
 * imports no kernel API, and is never loaded or executed.
 */
...
__declspec(noinline) NTSTATUS DispatchDeviceControl(
    ...
    if (stack->Parameters.DeviceIoControl.IoControlCode == 0x222004UL) {
        ...
        if (attacker_length > input_length - sizeof(ULONG)) {
            return 0;
        }
        if (attacker_length > sizeof(KernelSink)) {
            return 0;
        }
        RtlCopyMemory(
            (void *)KernelSink,
