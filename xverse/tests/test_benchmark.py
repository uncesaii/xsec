...
and assert the winner logic is honest — a xverse win on a gated target, a tie
...
def test_parse_roundtrip_and_schema_guard() -> None:
    text = _ndjson([_trial("t", "xverse", crash=True, ttc=3.0)])
    trials = benchmark.parse_results(text)
    assert len(trials) == 1 and trials[0].lane == "xverse"


def test_zeroverse_wins_gated_target() -> None:
    rows = [
        _trial("magic_gated", "xverse", crash=True, ttc=4.0, confirmed=True),
        _trial("magic_gated", "baseline", crash=False, ttc=None),
    ]
    comps = benchmark.summarize(benchmark.parse_results(_ndjson(rows)))
    assert len(comps) == 1 and comps[0].winner == "xverse"


def test_tie_inside_noise_floor() -> None:
    rows = [
        _trial("ungated", "xverse", crash=True, ttc=1.5),
...
def test_baseline_can_win() -> None:
    rows = [
        _trial("flat", "xverse", crash=True, ttc=20.0),
...
def test_neither_cracks() -> None:
    rows = [
        _trial("hard", "xverse", crash=False, ttc=None),
...
def test_tally_and_table() -> None:
    rows = [
        _trial("a", "xverse", crash=True, ttc=2.0), _trial("a", "baseline", crash=False, ttc=None),
        _trial("b", "xverse", crash=True, ttc=2.0), _trial("b", "baseline", crash=True, ttc=2.3),
    ]
    comps = benchmark.summarize(benchmark.parse_results(_ndjson(rows)))
    score = benchmark.tally(comps)
    assert score["xverse"] == 1 and score["tie"] == 1
...
def test_incompatible_schema_rejected() -> None:
    bad = json.dumps({"schema_version": "2.0", "target": "x", "lane": "xverse",
